...
- For count data, each bar is usually one or more integer values, rather than a range of rcontinuous values
...
> The Gaussian ditribution is sometimes referred to as the Normal distribution.  This is not a good practice: do not refer to the Gaussian distribution as the Normal distribution.  Referring to the Gaussian distribution as the normal distribution implies that Gaussian is "typical", which is patently untrue. <said in the voice of The Comic Book Guy, The Simpsons>
