...
**5** Every official R package has a webpage on the Comprehensive R Archive Network (CRAN) and there are often tutorials called "vignettes".  Google the CRAN page for the package {yarrr} and find the vignette called "Package guide".  Near the top is a demonstration for the function piratepal().  Read this and find a pallette you like aesthetically. Provide commented code for the steps required to display the vector for your favorite pallette.  (hint: the help page for piratepal() also has some example code to display the pallettes)
