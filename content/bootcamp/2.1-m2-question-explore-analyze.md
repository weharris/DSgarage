...
**2**  ```pct```, ```wet``` and ```dry``` should be numeric; ```block``` and ```trial``` should be factors, and ```treatment``` should be a factor with the level "Control" set as the reference.  Should the code to do this. 
